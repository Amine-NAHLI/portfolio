...
import { X, Shield, Code, Activity } from "lucide-react";
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 md:p-8">
          ...
          <motion.div
            ...
            className="relative w-full max-w-5xl h-full max-h-[85vh] bg-bg-1 border border-[var(--border)] rounded-[3rem] shadow-2xl overflow-hidden flex flex-col md:flex-row"
          >
            {/* Left Column: Visuals & Scanning */}
            <div className="w-full md:w-2/5 bg-bg-2 border-r border-[var(--border)] p-8 flex flex-col justify-between relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <div className="space-y-6">
                  <div className="p-6 rounded-3xl bg-text-1/[0.02] border border-[var(--border)]">
              ...
              <div className="relative z-10 pt-8 border-t border-[var(--border)]">
                ...
                <div className="flex gap-4">
                  <a
                    ...
                    target="_blank"
                    className="flex-1 flex items-center justify-center gap-3 py-4 bg-text-1/5 hover:bg-accent-cyan hover:text-[#0D1117] rounded-2xl transition-all font-bold text-xs uppercase tracking-widest text-text-2"
                  >
            ...
            <div className="flex-1 p-8 md:p-12 overflow-y-auto custom-scrollbar relative">
              <button
                onClick={onClose}
                className="absolute top-8 right-8 p-3 rounded-full bg-text-1/5 text-text-4 hover:text-text-1 hover:bg-text-1/10 transition-all"
              >
                <X size={20} />
              </button>
                  ...
                  <section>
                    ...
                    <div className="flex flex-wrap gap-3">
                      {project.tags.map(tag => (
                        <span key={tag} className="px-4 py-2 rounded-xl bg-text-1/5 border border-[var(--border)] text-text-3 text-xs font-mono">
                          {tag}
                        </span>
                      ))}
                    </div>
