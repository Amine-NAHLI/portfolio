    ...
    <div ref={ref} className="relative pl-12 md:pl-0 grid grid-cols-1 md:grid-cols-2 gap-12 mb-40 last:mb-0">
      ...
      <div className="hidden md:flex justify-end pr-24 items-start pt-2">
        <motion.span
          ...
          className="text-9xl font-black tracking-tighter text-text-1 tabular-nums"
        >
          {event.year}
        </motion.span>
      </div>
      ...
      <div className="relative">
        {/* Mobile Year */}
        <span className="md:hidden block text-6xl font-black text-text-1/5 mb-6 tabular-nums">{event.year}</span>
        ...
        <motion.div
          ...
          className={cn(
            "p-10 rounded-[2.5rem] border transition-all duration-1000 group hover:scale-[1.02] hover:-translate-y-1",
        ...
        >
          <div className="flex items-center gap-4 mb-8">
            <div className={cn(
              ...
              isInView ? "bg-cyan text-bg-0 shadow-lg shadow-cyan/20" : "bg-white/5 text-text-4"
            )}>
              <Icon size={20} />
            </div>
            <h3 className="text-2xl font-bold tracking-tight text-text-1">{event.title}</h3>
          </div>
          
          <p className="text-text-3 text-lg leading-relaxed mb-8 group-hover:text-text-2 transition-colors">
            {event.description}
          </p>

          {event.highlight && (
            <div className="flex items-center gap-2 font-mono text-[9px] uppercase tracking-[0.3em] text-cyan">
              <Star size={12} className="fill-cyan" />
              Strategic Milestone
            </div>
          )}
        </motion.div>
        ...
        <div className="absolute left-[-48px] md:left-[-6px] top-12 w-3 h-3 rounded-full border-2 border-bg-0 z-10 bg-white/10 overflow-hidden">
          <motion.div 
            ...
            className="w-full h-full bg-cyan shadow-[0_0_15px_var(--color-accent-cyan)]"
          />
        </div>
      </div>
    </div>
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          ...
          <p className="max-w-md text-text-3 text-lg leading-relaxed">
            A chronological roadmap of technical evolution and professional milestones.
          </p>
        </motion.div>
        ...
        <div ref={containerRef} className="relative max-w-6xl mx-auto">
          ...
          <div className="absolute left-0 md:left-1/2 top-0 bottom-0 w-px bg-white/5 md:-translate-x-1/2">
            <motion.div
              style={{ scaleY }}
              className="absolute top-0 left-0 right-0 h-full origin-top bg-gradient-to-b from-cyan via-indigo to-purple shadow-[0_0_20px_var(--color-accent-cyan)] opacity-50"
            />
          </div>
