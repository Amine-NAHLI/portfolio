...
export default function SiteHeader({ locale, dictionary, resumeLink }: SiteHeaderProps) {
  ...
  useEffect(() => { const dialog = dialogRef.current; if (!dialog) return; if (menuOpen && !dialog.open) dialog.showModal(); else if (!menuOpen && dialog.open) dialog.close(); }, [menuOpen]);
  function closeMenu() { setMenuOpen(false); }
  function rememberLocale() { localStorage.setItem("portfolio-locale", alternateLocale); const secure = window.location.protocol === "https:" ? "; Secure" : ""; document.cookie = `portfolio-locale=${alternateLocale}; Path=/; Max-Age=31536000; SameSite=Lax${secure}`; }

  return <header className="sticky top-0 z-50 border-b border-border bg-bg-page/85 backdrop-blur-xl supports-[backdrop-filter]:bg-bg-page/70">
    <Container className="flex min-h-[4.5rem] items-center justify-between gap-3">
      <Link href={`/${locale}`} className="group inline-flex min-h-11 items-center gap-3 rounded-sm text-text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent" aria-label={`${siteConfig.name} — ${dictionary.nav.home}`}>
        <span className="grid size-9 place-items-center border border-border-strong bg-surface text-xs font-bold tracking-[.12em] text-accent transition-transform duration-200 group-hover:-rotate-3">AN</span>
        <span className="hidden text-sm font-semibold tracking-tight sm:block">{siteConfig.name}<span className="ml-2 font-mono text-[.62rem] font-medium tracking-[.12em] text-text-muted">{"// 01"}</span></span>
      </Link>
      <nav className="hidden items-center gap-1 xl:flex" aria-label={dictionary.navigationLabel}>{navigation.map((item) => {
        ...
        return <LinkComponent key={item.href} href={item.href} aria-current={active ? "page" : undefined} className={cn("relative px-3 py-2 font-mono text-[.72rem] font-medium uppercase tracking-[.08em] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent after:absolute after:bottom-0 after:left-3 after:right-3 after:h-px after:origin-left after:bg-accent after:transition-transform", active ? "text-text-primary after:scale-x-100" : "text-text-secondary hover:text-text-primary after:scale-x-0")}>{item.label}</LinkComponent>;
      })}</nav>
