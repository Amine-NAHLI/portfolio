    ...
    <section id="stack" className="relative py-32 bg-transparent overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-12 mb-24">
          ...
          <div className="flex flex-wrap gap-2 p-2 rounded-[2.5rem] bg-bg-1 border border-bg-3 dark:bg-transparent dark:border-[#1e293b] backdrop-blur-2xl transition-colors">
             {categories.map((cat) => (
               <button
                 key={cat.id}
                 ...
                 className={`relative px-8 py-4 rounded-full font-mono text-[10px] uppercase tracking-widest transition-all duration-700 flex items-center gap-3 overflow-hidden ${
                   activeTab === cat.id ? "text-bg-0 dark:text-accent-cyan" : "text-text-2 dark:text-[#64748b] hover:text-text-1"
                 }`}
               >
                 {activeTab === cat.id && (
                   <motion.div
                     layoutId="activeTab"
                     className="absolute inset-0 bg-text-1 dark:bg-[#1e3a5f] rounded-full z-0"
        ...
        <div className="relative">
                ...
                {filteredTech.map((tech, i) => {
                  ...
                  return (
                    <motion.button
                      ...
                      className={`group relative p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] border transition-all duration-500 flex flex-col items-center justify-center gap-4 md:gap-6 text-center overflow-hidden min-h-[160px] md:min-h-[180px] ${
                        activeTech === tech.name 
                          ? "bg-text-1 border-text-1 shadow-[0_20px_50px_rgba(0,0,0,0.4)]" 
                          : "bg-bg-1/40 backdrop-blur-md border-text-1/10 hover:border-white/20"
                      }`}
                    >
                      <div className="absolute inset-0 opacity-0 group-hover:opacity-10 transition-opacity duration-700 pointer-events-none blur-[40px]" style={{ background: `radial-gradient(circle at center, ${accentColor}, transparent)` }} />
                      <div className={`w-14 h-14 md:w-16 md:h-16 rounded-2xl flex items-center justify-center transition-all duration-500 shadow-inner relative overflow-hidden ${activeTech === tech.name ? "bg-bg-0 text-text-1" : "bg-bg-2/50 text-text-4 group-hover:text-text-1 group-hover:bg-bg-2"}`}>
                        <IconComp size={24} className="relative z-10 transition-transform group-hover:scale-110" />
                        <div className="absolute inset-0 rounded-2xl opacity-10 group-hover:opacity-30 blur-md transition-opacity" style={{ backgroundColor: accentColor }} />
                      </div>
                      <div className="space-y-1 md:space-y-2 relative z-10">
                         <p className={`text-xs md:text-base font-black tracking-tighter leading-none transition-colors ${activeTech === tech.name ? "text-bg-0" : "text-text-1 group-hover:text-accent-cyan"}`}>{tech.name}</p>
                         <div className="flex items-center justify-center gap-2">
                            <div className="w-1 h-1 rounded-full animate-pulse" style={{ backgroundColor: accentColor }} />
                            <p className={`font-mono text-[8px] md:text-[9px] uppercase tracking-widest ${activeTech === tech.name ? "text-bg-0/60" : "text-text-4 group-hover:text-text-3"}`}>{tech.cat}</p>
                         </div>
                      </div>
                      <div className="absolute bottom-0 left-0 h-1 bg-gradient-to-r from-transparent via-accent-cyan to-transparent w-0 group-hover:w-full transition-all duration-700 opacity-50" />
                      {activeTech === tech.name && (
                        <motion.div layoutId="activeRing" className="absolute inset-0 rounded-[2rem] md:rounded-[2.5rem] border-2 border-white/20 pointer-events-none" />
                      )}
                    </motion.button>
                  );
                })}
          ...
          {filteredTech.length === 0 && (
            <div className="flex flex-col items-center justify-center py-48 space-y-8">
               ...
               <p className="font-mono text-sm text-text-4 uppercase tracking-[0.5em] text-center">No dynamic records found.</p>
            </div>
          )}
        </div>
      </div>
