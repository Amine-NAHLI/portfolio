...
import { getPublicCertifications, getPublicJourney, getPublicSkillGroups } from "@/features/portfolio/data";
...
export default async function SearchPage({ params }: SearchPageProps) {
  ...
  const [projects, skillGroups, certifications, timelineEntries] = await Promise.all([getPublishedProjects(locale), getPublicSkillGroups(locale), getPublicCertifications(locale), getPublicJourney(locale)]);
  const entries: PublicSearchEntry[] = [
    ...
    ...skillGroups.map((group) => ({ id: group.id, type: "skill" as const, title: group.title, description: group.description ?? "", href: `/${locale}/skills`, keywords: group.skills.map((skill) => skill.name) })),
