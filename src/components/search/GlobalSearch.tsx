...
export function GlobalSearch({ entries, locale }: { entries: PublicSearchEntry[]; locale: Locale }) {
  ...
  return (
    <div>
      ...
      {normalized.length >= 2 && results.length === 0 ? <div className="mt-4 border border-dashed border-border-strong p-10 text-center text-text-muted">{copy.empty}</div> : (
        <ul className="mt-4 grid gap-3">{results.map(({ entry }, index) => <li key={`${entry.type}:${entry.id}`}><Link href={entry.href} className="technical-frame group flex min-h-28 items-start justify-between gap-5 p-5 hover:border-border-strong"><div><p className="font-mono text-xs uppercase tracking-wider text-accent">{String(index + 1).padStart(2, "0")} {"// "}{copy.types[entry.type]}</p><h2 className="mt-2 text-lg font-semibold group-hover:text-accent">{entry.title}</h2><p className="mt-1 line-clamp-2 text-sm leading-6 text-text-muted">{entry.description}</p></div><ArrowUpRight aria-hidden="true" className="mt-1 size-4 shrink-0 text-text-muted" /></Link></li>)}</ul>
      )}
    </div>
  );
}
