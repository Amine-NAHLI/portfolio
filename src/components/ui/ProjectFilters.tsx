...
export default function ProjectFilters({ activeCategory, setActiveCategory }: FilterProps) {
  return (
    <div className="flex flex-col gap-6 w-full lg:w-48 shrink-0">
          ...
          return (
            ...
            >
              ...
              <div className={`w-1.5 h-8 rounded-full transition-all duration-500 ${
                isActive
                  ? "bg-accent-cyan shadow-[0_0_15px_rgba(0,180,216,0.5)]"
                  : "bg-[var(--border)]"
              }`} />
              ...
              {!isActive && (
                <div className="absolute inset-0 bg-text-1/[0.02] opacity-0 group-hover:opacity-100 rounded-xl transition-opacity" />
              )}
            </button>
          );
        })}
      ...
      {/* Connection Metadata */}
      <div className="hidden lg:flex flex-col gap-4 mt-8 pt-8" style={{ borderTop: "1px solid var(--border)" }}>
