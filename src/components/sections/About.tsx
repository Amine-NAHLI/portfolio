...
import { Shield, Brain, Fingerprint, Activity, GraduationCap, Code2, Globe, Music, BookOpen, Coffee } from "lucide-react";
    ...
    >
      <div className="relative w-24 h-24">
        <svg className="w-full h-full -rotate-90">
          <circle cx="48" cy="48" r={radius} className="stroke-text-1/5 fill-none" strokeWidth="4" />
          <motion.circle
        ...
        <div className="absolute -inset-2 border border-[var(--border)] rounded-full rotate-45 group-hover:rotate-180 transition-transform duration-1000" />
      </div>

      <div className="text-center space-y-1">
        <div className="font-mono text-[9px] uppercase tracking-[0.2em] text-accent-cyan font-black">{label}</div>
        <div className="px-2 py-0.5 rounded-sm bg-text-1/5 border border-[var(--border)] text-[8px] font-mono text-text-4 uppercase tracking-widest">
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
          ...
          <div className="relative">
            ...
            <div className="relative z-10 bg-bg-1/40 border border-[var(--border)] rounded-[4rem] p-12 backdrop-blur-3xl shadow-2xl">
              ...
              <div className="mt-12 space-y-4">
                <div className="flex justify-between items-center px-6 py-4 rounded-2xl bg-text-1/[0.02] border border-[var(--border)]">
                  <span className="text-[10px] font-mono text-text-4 uppercase">Cognitive_Load</span>
                  <div className="flex gap-1">
                    {[1, 2, 3, 4, 5, 6, 7, 8].map(i => (
                      <div key={i} className={`w-1.5 h-3 rounded-sm ${i < 7 ? 'bg-accent-cyan' : 'bg-text-1/10'}`} />
                    ))}
                  </div>
                </div>
              </div>
          ...
          <div className="space-y-10">
            ...
            <div className="terminal-dark rounded-3xl p-8 border shadow-2xl font-mono text-sm">
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                {[
                  ...
                  <div key={label} className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-text-1/[0.03] border border-[var(--border)] hover:border-accent-cyan/20 transition-colors">
        ...
        <div className="mt-24 grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="flex items-center gap-5 px-8 py-6 rounded-[2rem] bg-bg-1/40 border border-[var(--border)] backdrop-blur-xl">
          ...
          <a
            ...
            className="flex items-center gap-5 px-8 py-6 rounded-[2rem] bg-bg-1/40 border border-[var(--border)] backdrop-blur-xl hover:border-accent-cyan/20 transition-colors group"
          >
            <div className="w-12 h-12 rounded-2xl bg-text-1/5 border border-[var(--border)] flex items-center justify-center flex-shrink-0 group-hover:border-accent-cyan/30 transition-colors">
